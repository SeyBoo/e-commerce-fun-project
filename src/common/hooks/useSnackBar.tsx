...
const SnackBarProvider: FunctionComponent<PropsWithChildren> = ({
  ...
  const setSnackBar = ({ description, title, type }: useSnackProps) => {
    console.log("called");
  ...
  return (
    <SnackBarContext.Provider value={{ useSnack: setSnackBar }}>
      <>
        {children}
        <div className="absolute top-0 right-0">{component}</div>
      </>
